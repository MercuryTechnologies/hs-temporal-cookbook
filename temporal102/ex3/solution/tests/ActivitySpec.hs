...
spec = describe "test Activities" $ do
  ...
  describe "billCustomer" $ do
    ...
    it "returns the expected results for a weekly special test order" $ do
      mockEnv <- mkMockActivityEnvironment ()
      res <- runMockActivity mockEnv $ billCustomer mkSpecialTestOrder
      res.ocOrderNumber `shouldBe` mkBasicTestOrder.poOrderNumber
      res.ocAmountCents `shouldBe` 3400
...
mkSpecialTestOrder :: PizzaOrder
mkSpecialTestOrder = PizzaOrder addr 12345 [lgCheese, lgCheese, lgCheese]
  where
    addr = CustomerAddress "917 Delores Street" ""
    lgCheese = Pizza "Large, with cheese" 1300
